...
/**
 * Checks for successful authentication of an admin to allow for accessing the resources that are meant to be accessible only for administrator users.
 */
public class AdminFilter implements Filter {
    /**
     * A LOGGER available for all the subclasses.
     */
    protected static final Logger LOGGER = LogManager.getLogger(AdminFilter.class, StringFormatterMessageFactory.INSTANCE);

    /**
     * The name of the admin attribute in the session
     */
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if (session == null) {
                ...
                final String admin = (String) session.getAttribute(ADMIN_ATTRIBUTE);
                final Integer sagra = (Integer) session.getAttribute(SAGRA_ATTRIBUTE);

                // there might exist a session but without any user in it
                if (admin == null || admin.isBlank() || sagra == null ) {

                    // invalidate the session
                    session.invalidate();

                    LOGGER.warn("Authentication required to access resource %s with method %s. Session %s exists but no admin or sagra found in session. Session invalidated.", req.getRequestURI(), req.getMethod(), session.getId());
